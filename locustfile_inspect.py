...
MIN_WAIT = int(os.getenv("MIN_WAIT") or 0.8)
MAX_WAIT = int(os.getenv("MAX_WAIT") or 1.2)
...
class InspectUser(FastHttpUser):
    ...
    def small_inspect(self):
        LOGGER.info(f"Sending small inspect to app {APP_ADDRESS}")
        with self.rest("POST", f"/inspect/{APP_ADDRESS}",data=SMALL_PAYLOAD) as resp:
            if resp.js is None:
                ...
                LOGGER.info(f"   small inspect reponse for {APP_ADDRESS} has processed inputs = {resp.js['processed_input_count']}")

    @tag('medium')
    @task
    def medium_inspect(self):
        LOGGER.info(f"Sending medium inspect to app {APP_ADDRESS}")
        with self.rest("POST", f"/inspect/{APP_ADDRESS}",data=MEDIUM_PAYLOAD) as resp:
            if resp.js is None:
                ...
                LOGGER.info(f"   medium inspect reponse for {APP_ADDRESS} has processed inputs = {resp.js['processed_input_count']}")

    @tag('large')
    @task
    def large_inspect(self):
        LOGGER.info(f"Sending large inspect to app {APP_ADDRESS}")
        with self.rest("POST", f"/inspect/{APP_ADDRESS}",data=LARGE_PAYLOAD) as resp:
            if resp.js is None:
                ...
                LOGGER.info(f"   large inspect reponse for {APP_ADDRESS} has processed inputs = {resp.js['processed_input_count']}")
